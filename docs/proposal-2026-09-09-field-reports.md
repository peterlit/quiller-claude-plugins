...
- **D6. Shipping the XCUITest driver** (agent 2 qa #2): deferred to 0.14.0, not declined — approved as the driver-contract item, specified in Part F. 0.13.0 ships the grant probe (A1.4) which makes the failure loud and early, and documents the driver fallback pattern.

## Part D — judgment calls flagged for Peter

- **J1. The generic XCUITest driver — RESOLVED (approved 2026-09-13).** Agent 2 built a working generic driver in Causeway (`.qa-loop/driver/`) and reports it removed the grant problem *and* cut screenshot volume 3.4×. Two facts settled the call: the source is readable in place at `~/Documents/src/cardgame/.qa-loop/driver/` (no delivery needed), and the "big folder" is 89.9MB of DerivedData cache around 420 lines of actual source (QADriver.swift 250, qa.py 70, start.sh/stop.sh/project.yml/README). Peter's constraint: the plugins must stay language/platform agnostic. Resolution: the reviewer is untouched (the driver belongs to the qa loop), the qa loop is already iOS-specific, and the driver ships as the first backend behind a platform-neutral contract — specified as qa-loop-tools 0.14.0 in Part F. It's also the stepping stone HANDOFF already names for Tier 3.
...
## Part F — qa-loop-tools 0.14.0: the driver contract + ios-xcuitest backend (approved shape)

Ships after the 0.13.0 pair, as its own release. The point of the structure is Peter's constraint: the plugin family stays platform-agnostic *above the driver line*, and everything platform-specific lives in a named backend directory with a clean seam.

1. **The driver contract** (documented in CONTROLS.md + the qa SKILL.md, spoken by all tester prompts): a driver is anything that serves these verbs against one device/session — `launch [K=V …]`, `activate`/`terminate`/`home`/`state`/`frame`, `tap`/`doubletap`/`press`/`drag`/`dragslow`/`swipe`, `type`/`key`, `shot <host-path>`, `find`/`findall`/`wait <id>`, `tapid`/`tapbtn`/`taptext`, `labels [kind] [substring]`, `alert`, `tree`, `rotate`, `sleep`/`ping`/`quit`. Replies are `OK …`/`ERR …`; coordinates are device points; element queries use accessibility identifiers/labels. The Causeway driver's README table is essentially this contract already — it gets adopted nearly verbatim.
2. **`qa-loop-tools/drivers/ios-xcuitest/`** — the Causeway driver generalized: bundle id becomes a required argument to `start.sh` (recorded in the per-udid state dir so `qa.py` needs nothing new); `/private/tmp/causeway-qa/` → `/private/tmp/qa-driver/<udid>/`; `dd/` (build cache) never ships and is created adjacent at runtime; names de-Causeway'd. Six files in, six files out — no rewrite.
3. **Skill integration**: Stage 0 prefers a serving driver over the MCP simulator tool (killing the per-device grant problem for autonomous runs); the A1.4 grant probe becomes "driver `ping` on each worker, else MCP real-tap". The 0.13.0 fallback text ("build an ad-hoc driver") becomes "start the shipped backend".
4. **The seam, made explicit**: `provision_workers.sh`, the regression-test-writer, and the ios-xcuitest backend are documented as the *iOS backend family*; SKILL text above the driver line speaks only contract verbs. A future `android-appium/` or `web-playwright/` backend implements the same verb table in its own directory and provides its own provisioner/regression-writer equivalents; nothing above the line changes.
5. Smoke test per ritual §5: build and serve the generalized backend against a scratch simulator with a stock Apple sample app's bundle id — `launch`, `tap`, `labels`, `shot`, `rotate` round-trip — proving no Causeway residue.
