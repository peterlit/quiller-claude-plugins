...
- **docs/CONTROLS.md:gemini-residual-understated** (introduced by a fix) — look here because: doc-only, fixed in closeout, and the open question is now CLOSED — the -s/--sandbox flag was verified live (2026-09-13, gemini-cli 0.59.0, macOS Seatbelt): reads are NOT restricted under permissive-open or restrictive-open, and restrictive-closed blocks API egress; flag not adopted, empty jail remains the isolation, docs updated to say "verified".
