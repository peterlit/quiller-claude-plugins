...
- **security/panel_review.py:consent-xdg-inside-checkout** (introduced by a fix) — look here because: THE ONE OPEN MAJOR — absolute XDG_CONFIG_HOME pointing inside the checkout still re-arms the bundled-archive attack; cheap containment (commonpath rejection) is sketched in BACKLOG.md and worth doing before 0.12.0 ships.
